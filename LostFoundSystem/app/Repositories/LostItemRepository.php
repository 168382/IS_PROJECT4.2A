...
class LostItemRepository extends BaseRepository
{
    ...
    public function search(array $filters): LengthAwarePaginator
    {
        ...
        $filtered = array_filter($records, function ($item) use ($filters) {
            ...
            if (!empty($filters['location'])) {
                if (!str_contains(strtolower($item['location_lost'] ?? ''), strtolower($filters['location']))) {
                    return false;
                }
            }

            if (!empty($filters['date'])) {
                if (($item['date_lost'] ?? '') !== $filters['date']) {
                    return false;
                }
            }
    ...
    {
        $all = $this->all();
        return $all->whereIn('status', ['lost', 'pending_claim'])->values();
    }
}
